...
const VIDEO_ID = 'jfKfPfyJRdk'; // Lofi Girl 24/7 stream - Nguồn stream YouTube phổ biến
const URL = `https://www.youtube.com/live_chat?is_popout=1&v=${VIDEO_ID}`;
const OUT_DIR = path.join(__dirname, 'youtube_icons');
...
https.get(URL, {
    ...
    res.on('end', () => {
        ...
        try {
            ...
            if (!dataStr) {
                console.error('❌ Không tìm thấy chữ window["ytInitialData"] = trong HTML.');
                return;
            }
            ...
            console.log(`🔍 Tìm thấy ${unique.length} custom emojis. Bắt đầu lưu trữ 40 icons...`);

            let count = 0;
            top40.forEach((item, index) => {
                ...
                const fileName = `${String(index + 1).padStart(2, '0')}_${name}.${ext}`;
